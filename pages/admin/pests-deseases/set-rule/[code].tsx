...
const Admin = ({ user, pestOrDesease, symptoms }: AdminProps) => {
  const [selectedSymptomData, setSelectedSymptomData] = useState<number[]>(
    () => {
      const _selectedSymptomData =
        pestOrDesease.PestsAndDeseasesHasSymptoms.map(
          (v: any) => v.symptomCode
        );
  ...
  const [selectedSymptomDataCF, setSelectedSymptomDataCF] = useState<number[]>(
    () => {
      const _selectedSymptomDataCF =
        pestOrDesease.PestsAndDeseasesHasSymptoms.map((v: any) => v.expertCF);
  ...
  const onSubmitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (selectedSymptomData.length === 0) {
      toast.error(
        "Sistem gagal menyimpan rule, silahkan pilih gejala terlebih dahulu",
        {
          duration: 5000,
        }
      );
    ...
    toast.promise(
      fetchCreatePestOrDesease(pestAndDeseaseCode)
        .then((res) => res.json())
        .then((res) => {
        ...
        .catch(() => {
          toast.error(
            "Sistem gagal menyimpan rule, ada kesalahan pada sistem",
            {
              duration: 5000,
            }
          );
  ...
  const handleSelectOneSymptom = (symptomCode: number) => {
    if (selectedSymptomData.find((v) => v === symptomCode)) {
      setSelectedSymptomData(
        selectedSymptomData.filter((v) => v !== symptomCode)
      );
    } else {
      ...
      <main className="safe-horizontal-padding my-[16px] md:my-[48px]">
        <div className="text-sm breadcrumbs">
          <ul>
            ...
            <li>
              ...
              Pengaturan Rule untuk [HP{pestOrDesease.code}]:{" "}
              {pestOrDesease.name}
            </li>
          </ul>
        </div>
        <form onSubmit={onSubmitHandler} ref={formRef}>
          <div className="flex items-center justify-between">
            ...
            <button
              className={`btn btn-primary ${fetchIsLoading ? "loading" : ""}`}
              type="submit"
                  ...
                  {symptoms.map((symptom: any, index: number) => (
                    <tr key={index}>
                      <th>
                        <label>
                          <input
                            ...
                            value={symptom.code}
                            onChange={() =>
                              handleSelectOneSymptom(symptom.code)
                            }
                            checked={
                              selectedSymptomData.find(
                                (v) => v === symptom.code
                              )
                                ? true
                                : false
                      ...
                      <td>
                        ...
                        <label
                          htmlFor={`modal-${symptom.code}`}
                          className="w-[110px] h-[100px]"
                        >
                          <Image
                            className="object-cover rounded-md w-[110px] h-[100px]"
                      ...
                      <td>
                        <select
                          ...
                          name={symptom.code}
                          disabled={
                            selectedSymptomData.find((v) => v === symptom.code)
                              ? false
                              : true
                          }
                          defaultValue={
                            selectedSymptomDataCF.find(
                              (v, i) =>
                                i ===
                                selectedSymptomData.findIndex(
                                  (v) => v === symptom.code
                                )
                            )
                              ? selectedSymptomDataCF.find(
