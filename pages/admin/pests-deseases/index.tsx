...
const Admin = ({ user, _pestsDeseases }: AdminProps) => {
  ...
  const handleDeleteSelectedPestsAndDeseases = async () => {
    ...
    toast.promise(
      fetchDeletePestAndDesease()
        .then((res) => res.json())
        .then((res) => {
          setFetchIsLoading(false);
          setPestsDeseases(
            pestsDeseases.filter(
              (pd: any) => !selectedPestsDeseases.includes(pd.code)
            )
          );
        ...
        .catch(() => {
          toast.error(
            "Sistem gagal menghapus data, ada kesalahan pada sistem",
            {
              duration: 5000,
            }
          );
  ...
  const handleSelectOnePestDesease = (pestDeseaseCode: number) => {
    if (selectedPestsDeseases.find((v) => v === pestDeseaseCode)) {
      setSelectedPestsDeseases(
        selectedPestsDeseases.filter((v) => v !== pestDeseaseCode)
      );
    } else {
    ...
    <>
      ...
      <main className="safe-horizontal-padding my-[16px] md:my-[48px]">
        ...
        <div className="flex flex-col flex-wrap items-start justify-between lg:items-center lg:flex-row">
          <h4 className="mb-2 text-xl font-bold">Data Hama dan Penyakit</h4>
          <div className="flex flex-row-reverse items-center justify-center gap-4 lg:flex-row">
            {selectedPestsDeseases.length > 0 && (
              <button
                ...
                onClick={handleDeleteSelectedPestsAndDeseases}
                disabled={fetchIsLoading}
              >
                Hapus {selectedPestsDeseases.length} Data
              </button>
            )}
            <Link
              className="btn btn-primary"
              href="/admin/pests-deseases/create"
            >
              <BsPlus size={24} />
              Tambah Data
            </Link>
                  ...
                  <th>
                    <label>
                      <input
                        ...
                        checked={
                          selectedPestsDeseases.length === pestsDeseases.length
                        }
                        disabled={fetchIsLoading}
                      />
                    </label>
                    ...
                    <tr key={index}>
                      <th>
                        <label>
                          <input
                            type="checkbox"
                            className="checkbox"
                            onChange={() => handleSelectOnePestDesease(pd.code)}
                            checked={
                              selectedPestsDeseases.find((v) => v === pd.code)
                                ? true
                                : false
                            }
                            disabled={fetchIsLoading}
                      ...
                      <td>
                        <span className="max-w-[100px] lg:max-w-[200px] overflow-x-auto flex flex-wrap">
                          {pd.PestsAndDeseasesHasSymptoms.length > 0 ? (
                            pd.PestsAndDeseasesHasSymptoms.map(
                              (item: any, index: number) => (
                                <span key={index}>
                                  G{item.symptoms.code}
                                  {index ===
                                  pd.PestsAndDeseasesHasSymptoms.length - 1 ? (
                                    ""
                                  ) : (
                                    <span>,&nbsp;</span>
                                  )}
                      ...
                      <td>
                        <span className="flex flex-row items-center justify-start gap-2">
                          <Link
                            href={`/admin/pests-deseases/edit/${pd.code}`}
                            className="btn btn-outline btn-info btn-xs"
                          >
                            Ubah
                          </Link>
                          <Link
                            href={`/admin/pests-deseases/set-rule/${pd.code}`}
