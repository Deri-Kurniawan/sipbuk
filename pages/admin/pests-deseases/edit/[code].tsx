...
const AdminEditPestOrDesease = ({ user, pestOrDesease }: AdminCreateProps) => {
  ...
  const [activeIngredient, setActiveIngredient] = useState<string>(
    pestOrDesease.activeIngredient
  );
  ...
  const onSubmitHandler = async (e: any, pestOrDeseaseCode: number) => {
    ...
    toast.promise(
      fetchCreatePestOrDesease(pestOrDeseaseCode)
        .then((res) => res.json())
        .then((res) => {
        ...
        .catch(() => {
          toast.error(
            "Sistem gagal menyimpan pembaruan data, ada kesalahan pada sistem",
            {
              duration: 5000,
            }
          );
  ...
  return (
    <>
      ...
      <main className="safe-horizontal-padding my-[16px] md:my-[48px]">
        ...
        <div className="mt-2">
          <form
            onSubmit={(e: any) => onSubmitHandler(e, pestOrDesease.code)}
            ref={formRef}
          >
            <div className="form-control">
              ...
              <label className="input-group">
                <input
                  type="text"
                  name="name"
                  placeholder="Nama ... atau Hama ..."
                  className="w-full input input-bordered"
                  id="name"
                  required
                  disabled={fetchIsLoading}
                  defaultValue={pestOrDesease.name}
                />
              </label>
            </div>
            <div className="form-control">
