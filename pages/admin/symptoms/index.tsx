    ...
    toast.promise(
      fetchDeletePestAndDesease()
        ...
        .then((res) => {
          ...
          setSymptoms(
            symptoms.filter(
              (symptom: any) => !selectedSymptoms.includes(symptom.code)
            )
          );
          setSelectedSymptoms([]);
        })
    ...
    <>
      ...
      <main className="safe-horizontal-padding my-[16px] md:my-[48px]">
        ...
        <div className="flex flex-col flex-wrap items-start justify-between lg:items-center lg:flex-row">
          <h4 className="mb-2 text-xl font-bold">Data Gejala</h4>
          <div className="flex flex-row-reverse items-center justify-center gap-4 lg:flex-row">
            {selectedSymptoms.length > 0 && (
              <button
                ...
                onClick={handleDeleteSelectedSymptoms}
                disabled={fetchIsLoading}
              >
                Hapus {selectedSymptoms.length} Data
              </button>
            )}
            <Link className="btn btn-primary" href="/admin/symptoms/create">
              <BsPlus size={24} />
              Tambah Data
            </Link>
          </div>
                    ...
                    <tr key={index}>
                      <th>
                        <label>
                          <input
                            ...
                            checked={
                              selectedSymptoms.find((v) => v === symptom.code)
                                ? true
                                : false
                            }
                            disabled={fetchIsLoading}
                      ...
                      <td>
                        <div className="flex flex-row items-center justify-start gap-2">
                          <Link
                            href={`/admin/symptoms/edit/${symptom.code}`}
